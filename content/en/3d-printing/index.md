---
title: "3D printing"
description: "Prusa XL, Prusa MK4, several Enders and an Anycube resin printer with curing station: the 3D printing area at Starship Factory in Basel."
translationKey: "3d-printing"
---

3D printing is one of Starship Factory's main focuses. We have an extensive fleet of 3D
printers — from filament printers for sturdy parts to a resin printer for fine detail. Bring
your model along as an STL and slice it on site.

{{< columns count=2 >}}
{{< column >}}
## Prusa XL
The Prusa XL can print with two filaments at once — for two-colour parts, or for support
structures in a different material. It is also our printer for large workpieces.
{{< /column >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_prusa_xl.jpeg" alt="Prusa XL" loading="eager" fetchpriority="high" sizes="column" >}}
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_resin_anycubic.jpeg" alt="Anycube Photon Mono M7 Max" sizes="column" >}}
{{< /column >}}
{{< column >}}
## Anycube Photon Mono M7 Max
The Anycube Photon Mono M7 Max is a resin printer: it cures liquid resin layer by layer with
light. That gives considerably finer detail than a filament printer — ideal for figures,
model making and small, accurate parts.
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
## Resin curing station
Our resin curing station for the Anycube Photon Mono M7 Max. Freshly printed resin parts are
washed and post-cured in it — the step that turns a sticky blank into a finished part.
{{< /column >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_curing_station.jpeg" alt="Resin curing station" sizes="column" >}}
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_prusa_mk4.jpeg" alt="Prusa MK4" sizes="column" >}}
{{< /column >}}
{{< column >}}
## Prusa MK4
The Prusa MK4 is our workhorse for everyday prints in PLA and PETG: reliable, quick to set up
and a good first printer if you have never printed before.
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
## Ender
Several Ender 3D printers are available. They are well suited to printing several parts in
parallel, or letting a longer print run.
{{< /column >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_ender.jpeg" alt="Ender" sizes="column" >}}
{{< /column >}}
{{< /columns >}}

{{< spacer 20 >}}

Just drop by on one of our opening evenings and take a look — we are happy to show you how
the machines work. You do not need any previous experience.

{{< button link="/en/how-to-find-us/" text="How to find us" style="secondary" >}}
