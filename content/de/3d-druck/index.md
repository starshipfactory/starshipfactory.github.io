---
title: "3D-Druck"
description: "Prusa XL, Prusa MK4, mehrere Ender und ein Anycube-Resin-Drucker mit Curing Station: der 3D-Druck-Bereich der Starship Factory in Basel."
translationKey: "3d-printing"
---

3D-Druck ist ein Hauptschwerpunkt der Starship Factory. Wir verfügen über einen umfangreichen
Maschinenpark an 3D-Druckern — vom Filamentdrucker für robuste Teile bis zum Resin-Drucker
für feine Details. Dein Modell bringst du als STL mit, geschnitten wird vor Ort.

{{< columns count=2 >}}
{{< column >}}
## Prusa XL
Mit dem Prusa XL kann mit zwei Filamenten gleichzeitig gedruckt werden — für zweifarbige
Teile oder für Stützstrukturen aus einem anderen Material. Er ist auch unser Drucker für
grosse Werkstücke.
{{< /column >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_prusa_xl.jpeg" alt="Prusa XL" loading="eager" fetchpriority="high" sizes="column" >}}
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_resin_anycubic.jpeg" alt="Anycube Photon Mono M7 Max" sizes="column" >}}
{{< /column >}}
{{< column >}}
## Anycube Photon Mono M7 Max
Der Anycube Photon Mono M7 Max ist ein Resin-Drucker: Er härtet flüssiges Harz schichtweise
mit Licht aus. Das ergibt deutlich feinere Details als ein Filamentdrucker — ideal für
Figuren, Modellbau und kleine, genaue Teile.
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
## Resin Curing Station
Unsere Resin Curing Station für den Anycube Photon Mono M7 Max. Frisch gedruckte Resin-Teile
werden darin gewaschen und nachgehärtet — der Schritt, der aus einem klebrigen Rohling ein
fertiges Teil macht.
{{< /column >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_curing_station.jpeg" alt="Resin Curing Station" sizes="column" >}}
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_prusa_mk4.jpeg" alt="Prusa MK4" sizes="column" >}}
{{< /column >}}
{{< column >}}
## Prusa MK4
Der Prusa MK4 ist unser Arbeitstier für alltägliche Drucke aus PLA und PETG: zuverlässig,
schnell eingerichtet und ein guter erster Drucker, wenn du noch nie gedruckt hast.
{{< /column >}}
{{< /columns >}}

{{< columns count=2 >}}
{{< column >}}
## Ender
Diverse Ender 3D-Drucker stehen zur Verfügung. Sie sind gut geeignet, um parallel mehrere
Teile zu drucken oder einen längeren Druck laufen zu lassen.
{{< /column >}}
{{< column >}}
{{< img src="/img/3d-druck/3d_printer_ender.jpeg" alt="Ender" sizes="column" >}}
{{< /column >}}
{{< /columns >}}

{{< spacer 20 >}}

Komm einfach an einem Öffnungsabend vorbei und schau es dir an — wir zeigen dir gerne, wie
die Maschinen funktionieren. Vorkenntnisse brauchst du keine.

{{< button link="/anfahrt/" text="Anfahrt" style="secondary" >}}
